/**
 * Dia Argentina scraper, via the VTEX catalog API.
 *
 * Cencosud's Dia supermarket runs on the VTEX commerce platform.
 ...
 *
 *   - Dia overrides VTEX's default centavos representation, the
...
const API_BASE = "https://diaonline.supermercadosdia.com.ar";
...
// SEO metadata, navigation breadcrumbs, custom Dia attributes).
...
export type DiaProduct = z.infer<typeof DiaProductSchema>;

const DiaSearchResponseSchema = z.array(DiaProductSchema);
...
interface ArPicker {
  ...
  /**
   * Optional unit class parsed from the title. Dia mixes solid
...
const PICKERS: Partial<Record<ProductTarget["slug"], ArPicker>> = {
  ...
  potatoes_1kg: {
    // Dia AR doesn't surface "papa lavada" results; the bare "papa"
  ...
  chicken_breast_1kg: {
    // Dia AR sells the boneless chicken breast as "Suprema de Pollo"
...
/**
 * Parse a Spanish-grammar size token out of a Dia product name.
 *
...
export function parseSizeFromName(
  ...
  // Dia AR uses "Ud." (with trailing period) as its piece-count abbrev.
 ...
 * 1000 g and the unit class to "g": Dia prices loose produce and
 * butcher meat per kilogram regardless of `unitMultiplier`.
 *
 * Exported for unit tests.
 */
export function parseProduct(p: DiaProduct): ParsedProduct | null {
...
function pickBestMatch(
  ...
  // Dia's VTEX feed periodically serves corrupted products with
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = DiaSearchResponseSchema.safeParse(raw);
 ...
 */
export async function scrapeDiaAr(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("dia-ar");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `dia returned no candidates for "${picker.query}"`,
      });
      continue;
    }
