/**
 * Tavriav Ukraine scraper, zakaz.ua API.
 *
...
const TAVRIAV_KYIV_STORE_ID = "482211004";
...
const PICKERS: Partial<Record<ProductTarget["slug"], TavriavPicker>> = {
  bread_500g: {
    categoryId: "bakery-tavriav",
  ...
  milk_1l: {
    categoryId: "dairy-and-eggs-tavriav",
  ...
  tomatoes_1kg: {
    categoryId: "fruits-and-vegetables-tavriav",
  ...
  olive_oil_1l: {
    // Olive oil sits in Tavriav "Grocery" alongside flour, pasta and
    // dry goods. The dedicated `sauces-and-spices-tavriav` category
  ...
  chicken_breast_1kg: {
    categoryId: "meat-fish-poultry-tavriav",
...
async function fetchCategory(
  ...
  const url = `${API_BASE}/stores/${TAVRIAV_KYIV_STORE_ID}/categories/${categoryId}/products/?page=1&per_page=100`;
...
export function scrapeFromFixture(
  ...
  const targets = targetsForRetailer("tavriav-ua");
  ...
  for (const target of targets) {
    ...
    scraped.push({
      ...
      packSize: match.size,
      scrapedAt,
      sourceUrl: `${API_BASE}/stores/${TAVRIAV_KYIV_STORE_ID}/categories/${picker.categoryId}/products/?ean=${match.product.ean}`,
    });
  }

  return { retailer: "tavriav-ua", scraped, misses };
}
 ...
 */
export async function scrapeTavriavUa(
  fetchImpl: typeof fetch = fetch,
