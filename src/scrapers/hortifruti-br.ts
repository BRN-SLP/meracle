/**
 * Hortifruti Brazil scraper, via the VTEX catalog API.
 *
 * Hortifruti (Brazil) — mid-tier São Paulo supermarket runs on the same VTEX platform
...
const API_BASE = "https://www.hortifruti.com.br";
...
const HortifrutiProductSchema = z.object({
  productId: z.string(),
...
export type HortifrutiProduct = z.infer<typeof HortifrutiProductSchema>;

const HortifrutiSearchResponseSchema = z.array(HortifrutiProductSchema);
...
const PICKERS: Partial<Record<ProductTarget["slug"], PePicker>> = {
  ...
  butter_200g: {
    ...
    exclude: [
      ...
      // Hortifruti BR stocks "Pipoca Manteiga" (butter-flavoured
      // popcorn) and "Feijao Manteiga" (butter beans) which match
      // the bare /manteiga/ include without these explicit drops.
      /\b(?:pipoca|popcorn)\b/iu,
      /\bfeij[ãa]o\b/iu,
      // Garrafa = bottled liquid butter ("manteiga de garrafa"),
      // a sertanejo-specific render that's not equivalent to a
      // stick of butter.
      /\bgarrafa\b/iu,
    ],
  ...
  tomatoes_1kg: {
    ...
    exclude: [
      ...
      // BR: "Pelado" = peeled / canned-tomato-pulp, "Polpa" =
      // pulp, "Passata" = sieved sauce, all wrong for fresh
      // tomato. Mambo's Spanish-only exclude misses these.
      /\b(?:pelado|polpa|passata|seco|enlatado)\b/iu,
    ],
  ...
  potatoes_1kg: {
    ...
    // potato is "batata doce". Bolinha = baby potatoes. Hortifruti
  ...
  // Bonafont, São Lourenço, Prata. Hortifruti RJ stocks 510 ml individual
  ...
  cheese_local_500g: {
    ...
    exclude: [
      ...
      // BR: "Pao de queijo" is cheese-bread, not cheese.
      // "Requeijao" is spreadable cream cheese, different class.
      // "Creme de queijo" / "creme cheese" same story.
      /\bp[ãa]o\s+de\s+queijo\b/iu,
      /\brequeij[ãa]o\b/iu,
      /\bcreme\s+de?\s+queijo\b/iu,
    ],
...
export function parseProduct(p: HortifrutiProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = HortifrutiSearchResponseSchema.safeParse(raw);
...
export async function scrapeHortifrutiBr(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("hortifruti-br");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `hortifruti returned no candidates for "${picker.query}"`,
      });
      continue;
    }
