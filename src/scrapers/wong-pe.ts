/**
 * Wong Peru scraper, via the VTEX catalog API.
 *
...
const API_BASE = "https://www.wong.pe";
...
const WongProductSchema = z.object({
  productId: z.string(),
...
export type WongProduct = z.infer<typeof WongProductSchema>;

const WongSearchResponseSchema = z.array(WongProductSchema);
...
export function parseProduct(p: WongProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = WongSearchResponseSchema.safeParse(raw);
...
export async function scrapeWongPe(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("wong-pe");
