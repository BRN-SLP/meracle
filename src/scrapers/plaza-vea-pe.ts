/**
 * Plaza Vea Peru scraper, via the VTEX catalog API.
 *
 * Cencosud's Wong supermarket in Peru runs on the same VTEX platform
...
const API_BASE = "https://www.plazavea.com.pe";
...
const PlazaVeaProductSchema = z.object({
  productId: z.string(),
...
export type PlazaVeaProduct = z.infer<typeof PlazaVeaProductSchema>;

const PlazaVeaSearchResponseSchema = z.array(PlazaVeaProductSchema);
...
const PICKERS: Partial<Record<ProductTarget["slug"], PePicker>> = {
  ...
  water_bottled_1500ml: {
    // Plaza Vea PE's catalog does not stock a 1.5 L plain still water as
...
export function parseProduct(p: PlazaVeaProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = PlazaVeaSearchResponseSchema.safeParse(raw);
...
export async function scrapePlazaVeaPe(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("plaza-vea-pe");
