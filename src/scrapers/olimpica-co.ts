/**
 * Olimpica Colombia scraper, via the VTEX catalog API.
 *
 * Olimpica runs on the same VTEX platform as Disco AR and Wong PE.
 ...
 *
 * The Olimpica-specific quirk: it's a department store, every product
 ...
 *
 * Olimpica's search returns surprising cross-category hits because
...
const API_BASE = "https://www.olimpica.com";
...
const OlimpicaProductSchema = z.object({
  productId: z.string(),
...
export type OlimpicaProduct = z.infer<typeof OlimpicaProductSchema>;

const OlimpicaSearchResponseSchema = z.array(OlimpicaProductSchema);
...
export function parseProduct(p: OlimpicaProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = OlimpicaSearchResponseSchema.safeParse(raw);
...
export async function scrapeOlimpicaCo(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("olimpica-co");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `olimpica returned no candidates for "${picker.query}"`,
      });
      continue;
    }
