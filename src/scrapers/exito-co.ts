/**
 * Éxito Colombia scraper, via the VTEX catalog API.
 *
 * Éxito runs on the same VTEX platform as Olimpica CO and the rest of Grupo Éxito.
...
const API_BASE = "https://www.exito.com";
...
const ExitoProductSchema = z.object({
  productId: z.string(),
...
export type ExitoProduct = z.infer<typeof ExitoProductSchema>;

const ExitoSearchResponseSchema = z.array(ExitoProductSchema);
...
export function parseProduct(p: ExitoProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = ExitoSearchResponseSchema.safeParse(raw);
...
export async function scrapeExitoCo(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("exito-co");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `exito returned no candidates for "${picker.query}"`,
      });
      continue;
    }
