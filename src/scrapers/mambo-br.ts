/**
 * Mambo Brazil scraper, via the VTEX catalog API.
 *
 * Mambo (Brazil) — mid-tier São Paulo supermarket runs on the same VTEX platform
...
const API_BASE = "https://www.mambo.com.br";
...
const MamboProductSchema = z.object({
  productId: z.string(),
...
export type MamboProduct = z.infer<typeof MamboProductSchema>;

const MamboSearchResponseSchema = z.array(MamboProductSchema);
...
export function parseProduct(p: MamboProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = MamboSearchResponseSchema.safeParse(raw);
...
export async function scrapeMamboBr(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("mambo-br");
