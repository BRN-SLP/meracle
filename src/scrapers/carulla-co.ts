/**
 * Carulla Colombia scraper, via the VTEX catalog API.
 *
 * Carulla runs on the same VTEX platform as Olimpica CO and the rest of Grupo Éxito.
 ...
 *
 * The Carulla-specific quirk mirrors Olimpica: VTEX department store, every product
 ...
 *
 * Carulla.s search inherits the same VTEX cross-category hits because
...
const API_BASE = "https://www.carulla.com";
...
const CarullaProductSchema = z.object({
  productId: z.string(),
...
export type CarullaProduct = z.infer<typeof CarullaProductSchema>;

const CarullaSearchResponseSchema = z.array(CarullaProductSchema);
...
export function parseProduct(p: CarullaProduct): ParsedProduct | null {
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = CarullaSearchResponseSchema.safeParse(raw);
...
export async function scrapeCarullaCo(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("carulla-co");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `carulla returned no candidates for "${picker.query}"`,
      });
      continue;
    }
