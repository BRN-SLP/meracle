/**
 * Disco Argentina scraper, via the VTEX catalog API.
 *
 * Cencosud's Disco supermarket runs on the VTEX commerce platform.
 ...
 *
 *   - Disco overrides VTEX's default centavos representation, the
...
const API_BASE = "https://www.disco.com.ar";
...
// SEO metadata, navigation breadcrumbs, custom Disco attributes).
...
const DiscoProductSchema = z.object({
  productId: z.string(),
...
export type DiscoProduct = z.infer<typeof DiscoProductSchema>;

const DiscoSearchResponseSchema = z.array(DiscoProductSchema);
...
interface ArPicker {
  ...
  /**
   * Optional unit class parsed from the title. Disco mixes solid
...
/**
 * Parse a Spanish-grammar size token out of a Disco product name.
 *
...
export function parseSizeFromName(
  ...
  // Pieces: " 12 Un", " 6 U", " 30 Un", " 12 Unidades"
  const pcs = s.match(/(?<![a-zA-Z%\d.,])(\d{1,3})\s*(?:un|u|unidades?)\b/i);
  if (pcs) {
 ...
 * 1000 g and the unit class to "g": Disco prices loose produce and
 * butcher meat per kilogram regardless of `unitMultiplier`.
 *
 * Exported for unit tests.
 */
export function parseProduct(p: DiscoProduct): ParsedProduct | null {
...
function pickBestMatch(
  ...
  // Disco's VTEX feed periodically serves corrupted products with
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = DiscoSearchResponseSchema.safeParse(raw);
 ...
 */
export async function scrapeDiscoAr(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("disco-ar");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `disco returned no candidates for "${picker.query}"`,
      });
      continue;
    }
...
function helper_2c4444(val: unknown): boolean {
