/**
 * Vea Argentina scraper, via the VTEX catalog API.
 *
 * Cencosud's Vea supermarket runs on the VTEX commerce platform.
 ...
 *
 *   - Vea overrides VTEX's default centavos representation, the
...
const API_BASE = "https://www.vea.com.ar";
...
// SEO metadata, navigation breadcrumbs, custom Vea attributes).
...
const VeaProductSchema = z.object({
  productId: z.string(),
...
export type VeaProduct = z.infer<typeof VeaProductSchema>;

const VeaSearchResponseSchema = z.array(VeaProductSchema);
...
interface ArPicker {
  ...
  /**
   * Optional unit class parsed from the title. Vea mixes solid
...
/**
 * Parse a Spanish-grammar size token out of a Vea product name.
 *
 ...
 * 1000 g and the unit class to "g": Vea prices loose produce and
 * butcher meat per kilogram regardless of `unitMultiplier`.
 *
 * Exported for unit tests.
 */
export function parseProduct(p: VeaProduct): ParsedProduct | null {
...
function pickBestMatch(
  ...
  // Vea's VTEX feed periodically serves corrupted products with
...
export async function fetchQueryProducts(
  ...
  try {
    ...
    const parsed = VeaSearchResponseSchema.safeParse(raw);
 ...
 */
export async function scrapeVeaAr(
  fetchImpl: typeof fetch = fetch,
): Promise<ScraperResult> {
  const targets = targetsForRetailer("vea-ar");
  ...
  for (const target of targets) {
    ...
    if (candidates.length === 0) {
      misses.push({
        target,
        reason: `vea returned no candidates for "${picker.query}"`,
      });
      continue;
    }
